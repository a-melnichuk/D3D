...
private:
	...
	ID3D10EffectTechnique* mBoxTech;
	ID3D10EffectMatrixVariable* mfxBoxWVPVar;
	ID3D10EffectMatrixVariable* mfxBoxWVar;
	ID3D10EffectMatrixVariable* mfxBoxWTexVar;
...
void PickingApp::initApp()
{
	...
	mBoxTech		= fx::BoxFX->GetTechniqueByName("BoxTech");
	mfxBoxWTexVar	= fx::BoxFX->GetVariableByName("gTexMtx")->AsMatrix();
	mfxBoxWVar		= fx::BoxFX->GetVariableByName("gWorld")->AsMatrix();
	mfxBoxWVPVar	= fx::BoxFX->GetVariableByName("gWVP")->AsMatrix();
	mfxLightVar          = fx::BoxFX->GetVariableByName("gLight");
	mfxEyePosVar         = fx::BoxFX->GetVariableByName("gEyePosW");
...
void PickingApp::drawScene()
{
	...
	if( mPickResult)
	{
		...
		mfxBoxWVPVar->SetMatrix((float*)&WVP);
		mfxBoxWVar->SetMatrix((float*)&mTreeWorld);
		mfxBoxWTexVar->SetMatrix((float*)&I);


		for(UINT i = 0; i < techDesc.Passes; ++i)
		{
			ID3D10EffectPass* pass = mBoxTech->GetPassByIndex(i);
			pass->Apply(0);
			mBox.draw();
		}
	}
...
void PickingApp::pick(int sx, int sy)
{
	...
	D3DXVECTOR3 min,max;
	mTreeMesh.getAABB(min,max);

	mPickResult = D3DXBoxBoundProbe(&min,&max,&rayOrigin,&rayDir);
	if(mPickResult)
	{
		mBox.init(md3dDevice,min,max);
	}
}