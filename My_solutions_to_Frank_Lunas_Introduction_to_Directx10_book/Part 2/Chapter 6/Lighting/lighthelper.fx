...
float toonSpotLight(float l)
{
return l;
	[branch]
	if(l>=0.0f && l <=0.1f)
		return 0;
	 	[branch]if(l > 0.1f && l<=0.8f)
		return 0.5f;
		[branch]if(l > 0.8f && l<1.0f)
		return 1.0f;
	return l;
}

float toonPointLight(float l)
{
...
float3 PointLight(SurfaceInfo v, Light L, float3 eyePos)
{
	...
	float diffuseFactor = toonPointLight(dot(lightVec, v.normal));
	[branch]
	if( diffuseFactor > 0.0f )
	{
		float specPower  = max(v.spec.a, 1.0f);
...
float3 Spotlight(SurfaceInfo v, Light L, float3 eyePos)
{
	...
	float s = toonSpotLight(pow(max(dot(-lightVec, L.dir), 0.0f), L.spotPower));
