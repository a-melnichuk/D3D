...
void TerrainApp::updateScene(float dt)
{
	...
	//D3DXVECTOR3 posC = GetCamera().position() + dt * GetCamera().look();
	//float y = mLand.getHeight( posC.x,posC.z );

	if(GetAsyncKeyState('A') & 0x8000)	GetCamera().strafe(-40.0f*dt);
	if(GetAsyncKeyState('D') & 0x8000)	GetCamera().strafe(+40.0f*dt);
	if(GetAsyncKeyState('W') & 0x8000)	GetCamera().walkLand(+20.0f*dt,mLand);
	if(GetAsyncKeyState('S') & 0x8000)	GetCamera().walkLand(20.0f*dt,mLand);
