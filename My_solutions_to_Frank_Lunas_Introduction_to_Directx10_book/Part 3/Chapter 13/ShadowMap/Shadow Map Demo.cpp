...
private:
	...
	ID3D10EffectShaderResourceVariable* mfxSlideProjMapVar;
...
void ShadowMapApp::initApp()
{
	...
	mfxSlideProjMapVar   = fx::ShadowFX->GetVariableByName("gSlideProjMap")->AsShaderResource();
	...
	D3DXMatrixOrthoLH(&mLightVolume,1.0f, 1.0f, 1.0f, 100.0f);
	//mLightVolume =  GetCamera().proj();
}
...
void ShadowMapApp::updateScene(float dt)
{
	...
	lightPos.x = 50.0f*cosf(0.25f*mTimer.getGameTime());
	lightPos.y = 1.0f;
	lightPos.z = 50.0f*sinf(0.25f*mTimer.getGameTime());

	D3DXMatrixLookAtLH(&mLightView, &lightPos,
		&D3DXVECTOR3(0.0f, 1.5f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
...
void ShadowMapApp::drawScene()
{
    ...
    for(UINT i = 0; i < techDesc.Passes; ++i)
    {
		...
		// draw floor
		//
		mfxSlideProjMapVar->SetResource(mFloorMapRV);
