...
void ParticleApp::initApp()
{
	...
	flares.push_back(L"water_particle.dds"); 
	...
	*/
	...
	mFountain.init(md3dDevice, fx::FountainFX, texArray, 10000); 
	*/

	
	vector<wstring> flares;
	flares.push_back(L"particle.dds"); 
	ID3D10ShaderResourceView* texArray = GetTextureMgr().createTexArray(L"flares", flares);
	mExplosion.init(md3dDevice, fx::ExplosionFX, texArray, 10000);

	D3DXVECTOR3 emitP(0.0f,15.0f,0.0f);
	mExplosion.setEmitPos(emitP);
...
void ParticleApp::updateScene(float dt)
{
	...
	//mFountain.update(dt, mTimer.getGameTime());
	mExplosion.update(dt, mTimer.getGameTime());
...
void ParticleApp::drawScene()
{
	...
	//mFountain.setEyePos(GetCamera().position());
	//mFountain.draw();

	mExplosion.setEyePos(GetCamera().position());
	mExplosion.draw();
