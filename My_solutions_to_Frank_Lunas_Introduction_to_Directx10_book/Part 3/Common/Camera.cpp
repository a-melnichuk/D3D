...
/*
void Camera::walkLand(float d,Terrain& terr)
{	
	
	D3DXVECTOR3 pos_next = mPosition + d*mLook;
	float next_y = terr.getHeight(pos_next.x,pos_next.z) + 5;
	D3DXVECTOR3 next(pos_next.x, next_y, pos_next.z);
	D3DXVECTOR3 tan = next - mPosition;
	D3DXVec3Normalize(&tan, &tan);
	mPosition += d*tan;

	
	D3DXVECTOR3 pos_next = mPosition + d*mLook;
	float y = terr.getHeight(mPosition.x,mPosition.z);
	mPosition += d*mLook;
	mPosition.y = y + 5;

		std::wostringstream out;   
		out.precision(6);
		out << L"Diff: x:" << abs(mLook.x - tan.x) << L"y:" <<abs(mLook.y - tan.y) << L" z:" <<abs(mLook.z - tan.z)<< L"\n" ;
