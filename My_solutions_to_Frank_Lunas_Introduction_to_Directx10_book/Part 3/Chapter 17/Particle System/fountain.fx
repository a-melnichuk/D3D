...
{	
	...
	{	
		...
		if( gIn[0].age > 0.0001f )
		{
			...
			int slices = 10;
			float theta = 0.0f;
			float dTheta = 2.0f*PI/15.0f;

			for(int i = 0.0f;i < 15;++i)
			{
				float3 vRandom = RandUnitVec3((float)i/15.0f);
				vRandom.x *= 0.5f;
				vRandom.z *= 0.5f;

				float3  cylVec = { cos(theta)+vRandom.x, 5.0f, sin(theta)+vRandom.z };

				Particle p;
				p.initialPosW = gEmitPosW.xyz;
				p.initialVelW = cylVec;
...
VS_OUT DrawVS(Particle vIn)
{
	...
	float opacity = 1.6f - smoothstep(0.0f, 1.0f, t/1.0f);
...
{	
	...
	if( gIn[0].type != PT_EMITTER )
	{
		...
		//
		float halfWidth  = 0.01f*gIn[0].sizeW.x;
		float halfHeight = 0.01f*gIn[0].sizeW.y;
