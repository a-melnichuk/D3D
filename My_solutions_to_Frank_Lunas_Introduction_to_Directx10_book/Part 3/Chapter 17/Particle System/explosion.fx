...
{	
	...
	{	
			...
			for(int i = 0;i < n;++i)
			{
				float3 vRandom = RandUnitVec3((float)i/(float)n);
				float rand = 0.5f*(vRandom.x + 1) + 1.0f; //x E [2,4]
				Particle p;
				p.initialPosW = gEmitPosW.xyz;
				p.initialVelW =  10.0f*vRandom;
				p.sizeW       = float2(3.0f, rand);
				p.age         = 0.0f;
				p.type        = PT_EMITTER_SECOND;
				ptStream.Append(p);
			}
	}
	else if(gIn[0].type == PT_EMITTER_SECOND && gIn[0].age > gIn[0].sizeW.y )
	{

			int n = 15;
			float t = gIn[0].age;
			float3 pos = 0.5f*t*t*gAccelW + t*gIn[0].initialVelW +  gIn[0].initialPosW;
			for(int i = 0;i < n;++i)
			{
				float3 vRandom = RandUnitVec3((float)i/(float)n);
				float rand = 0.5f*(vRandom.x + 1) + 2.0f; //x E [2,4]
				Particle p;
				p.initialPosW = pos;
				p.initialVelW =  10.0f*vRandom;
				p.sizeW       = float2(3.0f, rand);
	...
	{
		if( gIn[0].age <= gIn[0].sizeW.y )
			//gIn[0].initialPosW.y += 0.2f;
...
VS_OUT DrawVS(Particle vIn)
{
	...
	//float opacity = vIn.sizeW.y - smoothstep(0.0f, vIn.sizeW.y, t/vIn.sizeW.y);
...
{	
		...
		float halfHeight = 0.1f*gIn[0].sizeW.x;
